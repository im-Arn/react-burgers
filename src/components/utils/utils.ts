...
export const setDate = (createdAt: string) => {
...
export const sumPrice = (yummy: string[], allIngredients: TIngredientItem[]) => {
  let sum = 0;
  if (allIngredients) {
    yummy.forEach(ing => {
      if (ing !== null) {
        const foundIngredient = allIngredients.find(item => item._id === ing);
        if (foundIngredient) {
